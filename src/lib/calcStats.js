...
export function calcStats(clientes) {
  const totalLions = clientes.filter(c => c.categoria === "LIONS").reduce((s, c) => s + (c.minutos || 0), 0);
  const totalClub = clientes.filter(c => c.categoria === "CLUB").reduce((s, c) => s + (c.minutos || 0), 0);
  const totalOtros = clientes.filter(c => c.categoria === "OTROS").reduce((s, c) => s + (c.minutos || 0), 0);
  const totalBonificados = clientes.reduce((s, c) => s + Math.max(0, c.bonificados || 0), 0);
